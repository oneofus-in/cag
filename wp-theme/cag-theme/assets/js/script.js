...
(function () {
    ...
    /* ---------- Drawer accordion ----------
       Standard wp_nav_menu output has no toggle button, so inject one per
       parent item (li.menu-item-has-children). The parent link still
       navigates; the injected caret expands/collapses its .sub-menu. */
    if (drawer) {
        drawer.querySelectorAll('.drawer-nav .menu-item-has-children').forEach(li => {
            const link = li.querySelector(':scope > a');
            const sub  = li.querySelector(':scope > .sub-menu');
            if (!link || !sub) return;
            const btn = document.createElement('button');
            btn.type = 'button';
            btn.className = 'drawer-caret';
            btn.setAttribute('aria-label', 'פתח תת-תפריט');
        ...
        if (!reduce && window.ScrollTrigger) {
            ...
            */
            ...
            /* ===== Generic [data-anim] outside hero =====
               Use fromTo (not from): a plain from() reads the element's
               *current* values as its end state. ScrollTrigger auto-refreshes
               on window 'load' (fonts + lazy images settling), and on those
               refreshes a from() tween can re-render its hidden start and
               never play — leaving the section stuck at opacity:0. On desktop
               the pinned/scrubbed hero keeps ScrollTrigger updating so this is
               masked; on mobile there's no pin, so the reveals get stuck.
               Hard-coding the end state with fromTo makes it refresh-proof. */
            ...
            if (activePane) {
                gsap.fromTo(activePane.querySelectorAll('.w-card'),
                    { opacity: 0, y: 24 },
                    {
                        opacity: 1, y: 0,
